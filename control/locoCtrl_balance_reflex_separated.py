...
# -----------------------------------------------------------------------------------------------------------------
    def reset(self, params=None):
        ...
        self.balance_stim['r_leg'] = dict(zip(self.m_keys, 0.01*np.ones(len(self.m_keys))))
        self.reflex_stim['r_leg'] = dict(zip(self.m_keys, 0.01*np.ones(len(self.m_keys))))

        self.stim['l_leg'] = dict(zip(self.m_keys, 0.01*np.ones(len(self.m_keys))))
        self.balance_stim['l_leg'] = dict(zip(self.m_keys, 0.01*np.ones(len(self.m_keys))))
        self.reflex_stim['l_leg'] = dict(zip(self.m_keys, 0.01*np.ones(len(self.m_keys))))
...
# -----------------------------------------------------------------------------------------------------------------
    def get_stim(self, _stm):
        return np.array([_stm['r_leg']['HFL'], _stm['r_leg']['GLU'],
            _stm['r_leg']['HAM'], _stm['r_leg']['RF'],
            _stm['r_leg']['VAS'], _stm['r_leg']['BFSH'],
            _stm['r_leg']['GAS'], _stm['r_leg']['SOL'],
            _stm['r_leg']['TA'],
            _stm['l_leg']['HFL'], _stm['l_leg']['GLU'],
            _stm['l_leg']['HAM'], _stm['l_leg']['RF'],
            _stm['l_leg']['VAS'], _stm['l_leg']['BFSH'],
            _stm['l_leg']['GAS'], _stm['l_leg']['SOL'],
            _stm['l_leg']['TA']
            ])
...
# -----------------------------------------------------------------------------------------------------------------
    def update(self, sensor_data):
        ...
        return self.get_stim(self.stim)
...
# -----------------------------------------------------------------------------------------------------------------
    def _spinal_control(self, sensor_data):
        for s_leg in ['r_leg', 'l_leg']:
            ...
            self.stim[s_leg],self.balance_stim[s_leg], self.reflex_stim[s_leg] = self.spinal_control_leg(s_leg, sensor_data)
...
# -----------------------------------------------------------------------------------------------------------------
    def spinal_control_leg(self, s_leg, sensor_data):
        ...
        # @TODO jkr We want to separate descending signal for balance control and reflex circuitry.
        # The goal is that we can learn only spinal circuitry pattern with CPG. 
        # Ideally we should return a tuple of (stim, balance_stim) instead of just stim array.
        ...
        for muscle in stim:
            if(muscle in balance_stim):
                reflex_stim[muscle] = stim[muscle] - balance_stim[muscle]
            else:
                reflex_stim[muscle] = stim[muscle]
                balance_stim[muscle] = 0

        return stim, balance_stim, reflex_stim