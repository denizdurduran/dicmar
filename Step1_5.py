...
import random
import pickle
from mpi4py import MPI # SKIPPED
import time
import copy            # SKIPPED
import os
...
if is_worker():
    ...
    def loop(init,set_param):
        ...
        # Record joint angles
        JointLog = []
        joint_pos = env.get_observation_list_joints()
        ...
        if TEST:
            ...
            print("Saving logs to {}_[muscle_act|joint]_{}_second.pkl".format(SAVE_PATH.split('.pkl')[0], args.duration))
            musclePath = SAVE_PATH.replace('.pkl', '_' + args.duration + '_second' + '_muscle_act.pkl')
            jointPath = SAVE_PATH.replace('.pkl', '_' + args.duration + '_second_all' + '_joints.pkl')
