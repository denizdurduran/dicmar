from .env_wrappers import SPACE_TYPE,USE_LSTM
...
if(SPACE_TYPE >= 1):
    if(USE_LSTM):
        from .network_lstm import StateNet
    else:
        from .network_tiny import StateNet
else:
    from .network import StateNet


class SkeletonActor(ActorSpec):
    ...
    def get_from_params(
        ...
        if("out_features" not in policy_head_params):
